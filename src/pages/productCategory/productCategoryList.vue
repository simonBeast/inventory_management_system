<template>
  ...
  <div v-else-if="!errorMessage && authStore.isLoggedIn && authStore.isAdmin"
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ isLanguageTigrigna ? "መዝገብ ጨናፍር ጉጅለ" : "Product Categories" }}</h1>
      </div>
      <button @click="openCreateModal"
        ...
        {{ isLanguageTigrigna ? "ጨንፈር ጉጅለ ፍጠር" : "Create Product Category" }}
      </button>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mb-8">
      <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
        <div class="md:col-span-12">
          <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 px-1">{{ isLanguageTigrigna ? "ጨናፍር ጉጅለ ኣልሽ" : "Search Product Categories" }}</label>
        ...
        <table class="min-w-full table-auto">
          <thead class="bg-gray-50/50 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider font-semibold text-left border-b border-gray-200 dark:border-gray-700">
            <tr>
              ...
              <th class="px-6 py-4">{{ isLanguageTigrigna ? "ሽም ጨንፈር ጉጅለ" : "Name" }}</th>
              <th class="px-6 py-4">{{ isLanguageTigrigna ? "ቀንዲ ጉጅለ" : "Category" }}</th>
          ...
          <tbody class="text-gray-700 dark:text-gray-200 text-sm divide-y divide-gray-100 dark:divide-gray-800">
            <tr v-for="(category, index) in productCategories?.data || []" :key="category.id"
  ...
  <dialog id="deleteModal" class="modal">
    <div class="modal-box rounded-2xl shadow-2xl dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
      ...
      <p class="py-4 text-gray-600 dark:text-gray-300">
        {{ isLanguageTigrigna ? "እቲ ጨንፈር ጉጅለ ክተጥፈኦ ምድላይካ እርግጠኛ ዲኻ ?" : "Are you sure you want to delete this product category? This action cannot be undone." }}
      </p>
  ...
  <BaseModal :isOpen="isFormModalOpen" @close="closeFormModal" :title="formModalTitle" class="max-w-md">
    <template #body>
      <form @submit.prevent="submitForm" class="space-y-5">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ isLanguageTigrigna ? "ሽም ጨንፈር ጉጅለ" : "Category Name" }}</label>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ isLanguageTigrigna ? "ቀንዲ ጉጅለ" : "Main Category" }}</label>
          <select v-model="formData.categoryId" required
            ...
            <option v-for="cat in categoriesAlpha || []" :key="cat.id" :value="cat.id">{{ cat.name }}</option>
          </select>
        </div>
        <div>
...
import { useProductCategories, useDeleteProductCategory, useCreateProductCategory, useUpdateProductCategory } from '../../store/useProductCategory';
import { useCategoriesAlphaNoLimit } from '../../store/useCategory';
...
const { data: productCategories, isLoading, isError, error } = useProductCategories(params);
const { data: categoriesAlpha } = useCategoriesAlphaNoLimit(authStore.token);

const deleteMutation = useDeleteProductCategory(authStore.token);
const createMutation = useCreateProductCategory(authStore.token);
const updateMutation = useUpdateProductCategory(authStore.token);
...
const formModalTitle = computed(() => {
  return isEditing.value 
    ? (isLanguageTigrigna.value ? "ጨንፈር ጉጅለ ኣዐርይ" : "Edit Product Category") 
    : (isLanguageTigrigna.value ? "ሓዱሽ ጨንፈር ጉጅለ ፍጠር" : "Create New Product Category");
});
