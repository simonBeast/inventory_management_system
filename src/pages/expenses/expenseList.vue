<template>
  <span v-if="loading && !errorMessage" class="block loading loading-spinner text-blue-600 mt-8 mx-auto"></span>

  <div v-else-if="!errorMessage && authStore.isLoggedIn && authStore.isAdmin"
    class="p-4 md:p-8 w-full max-w-7xl mx-auto transition-all duration-300">

    <!-- Header -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ isLanguageTigrigna ? "Expenses" : "Expenses" }}</h1>
      </div>
      <button @click="openCreateModal"
        class="bg-gradient-to-r from-blue-600 to-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:from-blue-700 hover:to-blue-700 shadow-md hover:shadow-lg transition-all active:scale-95 flex items-center gap-2 self-start sm:self-auto">
        <font-awesome-icon icon="plus" />
        {{ isLanguageTigrigna ? "Add Expense" : "Add Expense" }}
      </button>
    </div>

    <!-- Filters Section -->
    <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mb-6">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
        <div class="lg:col-span-4">
          <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 px-1">{{ isLanguageTigrigna ? "Category" : "Category" }}</label>
          <select v-model="categoryFilter"
            class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 focus:ring-2 focus:ring-blue-500 outline-none text-sm transition-all text-gray-900 dark:text-white appearance-none">
            <option value="">{{ isLanguageTigrigna ? "All Categories" : "All Categories" }}</option>
            <option v-for="option in expenseCategoryOptions" :key="option.value" :value="option.value">
              {{ option.label }}
            </option>
          </select>
        </div>

        <div class="lg:col-span-5 grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 px-1">{{ isLanguageTigrigna ? "Start Date" : "Start Date" }}</label>
            <input v-model="fromDateFilter" type="date"
              class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 focus:ring-2 focus:ring-blue-500 outline-none text-sm transition-all text-gray-900 dark:text-white" />
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 px-1">{{ isLanguageTigrigna ? "End Date" : "End Date" }}</label>
            <input v-model="toDateFilter" type="date"
              class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 focus:ring-2 focus:ring-blue-500 outline-none text-sm transition-all text-gray-900 dark:text-white" />
          </div>
        </div>
      </div>
    </div>

    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 px-6 py-4 mb-8">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ isLanguageTigrigna ? "Total Expense" : "Total Expense" }}</div>
        <div class="text-2xl font-bold text-blue-600 dark:text-blue-400 font-mono">
          {{ totalExpense.toLocaleString() }} Birr
        </div>
      </div>
    </div>

    <!-- Table Section -->
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full table-auto">
          <thead class="bg-gray-50/50 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider font-semibold text-left border-b border-gray-200 dark:border-gray-700">
            <tr>
              <th class="px-6 py-4 w-16">#</th>
              <th class="px-6 py-4">{{ isLanguageTigrigna ? "Category" : "Category" }}</th>
              <th class="px-6 py-4">{{ isLanguageTigrigna ? "Amount" : "Amount" }}</th>
              <th class="px-6 py-4">{{ isLanguageTigrigna ? "Expense Date" : "Expense Date" }}</th>
              <th class="px-6 py-4">{{ isLanguageTigrigna ? "Description" : "Description" }}</th>
              <th class="px-6 py-4 text-right">{{ isLanguageTigrigna ? "Actions" : "Actions" }}</th>
            </tr>
          </thead>
          <tbody class="text-gray-700 dark:text-gray-200 text-sm divide-y divide-gray-100 dark:divide-gray-800">
            <tr v-for="(expense, index) in expenses?.data || []" :key="expense.id"
              class="hover:bg-blue-50/30 dark:hover:bg-gray-800/80 transition-colors group">
              <td class="px-6 py-4 font-medium text-gray-500">{{ (page - 1) * 10 + index + 1 }}</td>
              <td class="px-6 py-4 font-medium">
                {{ getCategoryLabel(expense.category) }}
              </td>
              <td class="px-6 py-4 font-mono font-semibold text-blue-600 dark:text-blue-400">
                {{ Number(expense.amount || 0).toLocaleString() }}
              </td>
              <td class="px-6 py-4 text-gray-500">{{ formatDate(expense.expenseDate) }}</td>
              <td class="px-6 py-4 text-gray-500">
                <span class="block max-w-xs truncate">{{ expense.description || '-' }}</span>
              </td>
              <td class="px-6 py-4 space-x-3 whitespace-nowrap text-right opacity-80 group-hover:opacity-100 transition-opacity">
                <button @click="handleEdit(expense)"
                  class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg text-blue-600 bg-blue-50 hover:bg-blue-600 hover:text-white dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-600 dark:hover:text-white transition-colors">
                  <font-awesome-icon icon="pen" class="w-3.5 h-3.5" />
                  {{ isLanguageTigrigna ? "Edit" : "Edit" }}
                </button>
                <button @click="openConfirmationModal(expense.id)"
                  class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg text-red-600 bg-red-50 hover:bg-red-600 hover:text-white dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-600 dark:hover:text-white transition-colors">
                  <font-awesome-icon icon="trash" class="w-3.5 h-3.5" />
                  {{ isLanguageTigrigna ? "Delete" : "Delete" }}
                </button>
              </td>
            </tr>
            <tr v-if="!expenses?.data || expenses.data.length === 0">
              <td colspan="6" class="px-6 py-12 text-center text-gray-500">{{ isLanguageTigrigna ? "No expense records found." : "No expense records found." }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>

    <!-- Pagination -->
    <div class="mt-8 flex justify-center">
      <pagination v-if="paginationData && paginationData.totalPages > 1"
        :total-pages="paginationData.totalPages"
        :current-page="paginationData.currentPage"
        @page-changed="handlePageChange" />
    </div>
  </div>

  <p v-if="errorMessage" class="text-center text-red-600 font-semibold mt-10 px-8">
    <font-awesome-icon icon="circle-exclamation" class="mr-2" /> {{ errorMessage }}
  </p>

  <!-- Delete Modal -->
  <dialog id="deleteModal" class="modal">
    <div class="modal-box rounded-2xl shadow-2xl dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
      <h3 class="font-bold text-xl text-gray-900 dark:text-white flex items-center gap-2">
        <font-awesome-icon icon="triangle-exclamation" class="text-red-500" />
        {{ isLanguageTigrigna ? "Confirm Deletion" : "Confirm Deletion" }}
      </h3>
      <p class="py-4 text-gray-600 dark:text-gray-300">
        {{ isLanguageTigrigna ? "Are you sure you want to delete this expense?" : "Are you sure you want to delete this expense?" }}
      </p>
      <div class="modal-action flex justify-end gap-3">
        <button @click="closeModal" class="px-4 py-2 rounded-xl bg-gray-200 text-gray-800 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 font-medium transition-colors">
          {{ isLanguageTigrigna ? "Cancel" : "Cancel" }}
        </button>
        <button @click="confirmDelete" class="px-4 py-2 rounded-xl bg-red-600 text-white hover:bg-red-700 font-medium shadow-sm transition-colors">
          {{ isLanguageTigrigna ? "Delete" : "Delete" }}
        </button>
      </div>
    </div>
    <form method="dialog" class="modal-backdrop">
      <button>close</button>
    </form>
  </dialog>

  <!-- Expense Form Modal -->
  <BaseModal :isOpen="isFormModalOpen" @close="closeFormModal" :title="formModalTitle" class="max-w-md">
    <template #body>
      <form @submit.prevent="submitForm" class="space-y-5">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ isLanguageTigrigna ? "Category" : "Category" }}</label>
          <select v-model="formData.category"
            class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-shadow appearance-none">
            <option v-for="option in expenseCategoryOptions" :key="option.value" :value="option.value">
              {{ option.label }}
            </option>
          </select>
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ isLanguageTigrigna ? "Amount" : "Amount" }}</label>
          <input v-model.number="formData.amount" type="number" required min="0" step="0.01"
            class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-shadow" />
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ isLanguageTigrigna ? "Expense Date" : "Expense Date" }}</label>
          <input v-model="formData.expenseDate" type="date"
            class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-shadow" />
          <p class="text-xs text-gray-400 mt-1">{{ isLanguageTigrigna ? "Leave empty to use today's date" : "Leave empty to use today's date" }}</p>
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ isLanguageTigrigna ? "Description" : "Description" }}</label>
          <textarea rows="3" v-model="formData.description"
            class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-shadow resize-none"></textarea>
        </div>

        <div v-if="formError" class="text-sm text-red-600 bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-800/30">
          <font-awesome-icon icon="circle-exclamation" class="mr-1" /> {{ formError }}
        </div>

        <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
          <button type="button" @click="closeFormModal" class="px-5 py-2.5 rounded-xl bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 font-medium transition-colors">
            {{ isLanguageTigrigna ? "Cancel" : "Cancel" }}
          </button>
          <button type="submit" :disabled="isSubmitting"
            class="px-8 py-2.5 rounded-xl bg-blue-600 text-white font-bold shadow-lg shadow-blue-100 dark:shadow-none hover:bg-blue-700 transition-all flex items-center gap-2 disabled:opacity-50">
            <span v-if="isSubmitting" class="loading loading-spinner loading-sm"></span>
            {{ isEditing ? (isLanguageTigrigna ? 'Update Expense' : 'Update Expense') : (isLanguageTigrigna ? 'Save Expense' : 'Save Expense') }}
          </button>
        </div>
      </form>
    </template>
  </BaseModal>
</template>

<script setup>
import { ref, computed, watch, reactive } from 'vue';
import { useRoute, useRouter } from 'vue-router';
import { useAuthStore } from '../../store/authStore';
import pagination from '../../components/pagination.vue';
import BaseModal from '../../components/BaseModal.vue';
import { useLanguageStore } from '../../store/languageStore';
import { useExpenses, useCreateExpense, useUpdateExpense, useDeleteExpense } from '../../store/useExpense';
import { useQueryClient } from '@tanstack/vue-query';

const route = useRoute();
const router = useRouter();
const authStore = useAuthStore();
const languageStore = useLanguageStore();
const queryClient = useQueryClient();

const isLanguageTigrigna = computed(() => languageStore.languagePreference == 'ti');

const expenseCategoryOptions = computed(() => ([
  { value: 'rent', label: isLanguageTigrigna.value ? 'Rent' : 'Rent' },
  { value: 'utilities', label: isLanguageTigrigna.value ? 'Utilities' : 'Utilities' },
  { value: 'transport', label: isLanguageTigrigna.value ? 'Transport' : 'Transport' },
  { value: 'salaries', label: isLanguageTigrigna.value ? 'Salaries' : 'Salaries' },
  { value: 'maintenance', label: isLanguageTigrigna.value ? 'Maintenance' : 'Maintenance' },
  { value: 'supplies', label: isLanguageTigrigna.value ? 'Supplies' : 'Supplies' },
  { value: 'equipment', label: isLanguageTigrigna.value ? 'Equipment' : 'Equipment' },
  { value: 'tax', label: isLanguageTigrigna.value ? 'Tax' : 'Tax' },
  { value: 'other', label: isLanguageTigrigna.value ? 'Other' : 'Other' }
]));

const page = ref(Number(route.query.page) || 1);
const categoryFilter = ref(route.query.category || '');
const fromDateFilter = ref(route.query.from || '');
const toDateFilter = ref(route.query.to || '');
const errorMessage = ref('');

const isFormModalOpen = ref(false);
const isEditing = ref(false);
const editingId = ref(null);
const isSubmitting = ref(false);
const formError = ref('');
const expenseIdToDelete = ref(null);

const formData = reactive({
  category: 'other',
  amount: 0,
  expenseDate: '',
  description: ''
});

const params = computed(() => ({
  page: page.value,
  limit: 10,
  token: authStore.token,
  category: categoryFilter.value,
  startDate: fromDateFilter.value,
  endDate: toDateFilter.value
}));

const { isLoading: loading, data: expenses, isError, error } = useExpenses(params);
const createMutation = useCreateExpense(authStore.token);
const updateMutation = useUpdateExpense(authStore.token);
const deleteMutation = useDeleteExpense(authStore.token);

const paginationData = computed(() => expenses.value?.pagination || null);
const totalExpense = computed(() => Number(expenses.value?.totalAmount || 0));

const formModalTitle = computed(() => {
  return isEditing.value
    ? (isLanguageTigrigna.value ? 'Edit Expense' : 'Edit Expense')
    : (isLanguageTigrigna.value ? 'Add Expense' : 'Add Expense');
});

function formatDate(value) {
  if (!value) return '-';
  const normalized = String(value).length === 10 ? `${value}T00:00:00` : value;
  return new Date(normalized).toLocaleDateString();
}

function getCategoryLabel(value) {
  const option = expenseCategoryOptions.value.find(item => item.value === value);
  return option?.label || value || '-';
}

function syncQuery() {
  router.replace({
    query: {
      ...route.query,
      page: page.value,
      category: categoryFilter.value || undefined,
      from: fromDateFilter.value || undefined,
      to: toDateFilter.value || undefined
    }
  });
}

function handlePageChange(newPage) {
  page.value = newPage;
  syncQuery();
}

function openCreateModal() {
  isEditing.value = false;
  editingId.value = null;
  formData.category = 'other';
  formData.amount = 0;
  formData.expenseDate = '';
  formData.description = '';
  formError.value = '';
  isFormModalOpen.value = true;
}

function handleEdit(expense) {
  isEditing.value = true;
  editingId.value = expense.id;
  formData.category = expense.category || 'other';
  formData.amount = Number(expense.amount || 0);
  formData.expenseDate = expense.expenseDate || '';
  formData.description = expense.description || '';
  formError.value = '';
  isFormModalOpen.value = true;
}

function closeFormModal() {
  isFormModalOpen.value = false;
}

function openConfirmationModal(id) {
  expenseIdToDelete.value = id;
  document.getElementById('deleteModal').showModal();
}

function closeModal() {
  document.getElementById('deleteModal').close();
}

function buildPayload() {
  const payload = {
    category: formData.category || 'other',
    amount: Number(formData.amount)
  };

  if (formData.expenseDate) {
    payload.expenseDate = formData.expenseDate;
  }

  if (formData.description !== undefined) {
    const trimmedDescription = formData.description?.trim();
    payload.description = trimmedDescription ? trimmedDescription : null;
  }

  return payload;
}

async function submitForm() {
  isSubmitting.value = true;
  formError.value = '';
  try {
    const payload = buildPayload();
    if (isEditing.value) {
      await updateMutation.mutateAsync({ id: editingId.value, data: payload });
    } else {
      await createMutation.mutateAsync(payload);
    }
    queryClient.invalidateQueries(['expenses']);
    closeFormModal();
  } catch (err) {
    formError.value = err.message;
  } finally {
    isSubmitting.value = false;
  }
}

async function confirmDelete() {
  try {
    await deleteMutation.mutateAsync(expenseIdToDelete.value);
    queryClient.invalidateQueries(['expenses']);
    closeModal();
  } catch (err) {
    errorMessage.value = err.message;
  }
}

watch([isError, error], ([hasError, err]) => {
  errorMessage.value = hasError ? (err.message || 'Something went wrong') : '';
});

watch([categoryFilter, fromDateFilter, toDateFilter], () => {
  page.value = 1;
  syncQuery();
});
</script>

<style scoped></style>
